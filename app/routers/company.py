...
def get_company(company_id: str, db: Session = Depends(get_db_context), user: User = Depends(AuthService.token_interceptor)):
...
def create_company(company: CompanyCreateModel, db: Session = Depends(get_db_context), user: User = Depends(AuthService.token_interceptor)):
    created_company = CompanyRepository.create_company(db, company)
    return created_company

@router.put("/{company_id}", response_model=CompanyViewModel)
def update_company(company_id: str, company: CompanyUpdateModel, db: Session = Depends(get_db_context), user: User = Depends(AuthService.token_interceptor)):
...
def delete_company(company_id: str, db: Session = Depends(get_db_context), user: User = Depends(AuthService.token_interceptor)):
...
def search_companies(search_params: CompanySearchModel, db: Session = Depends(get_db_context), user: User = Depends(AuthService.token_interceptor)):
